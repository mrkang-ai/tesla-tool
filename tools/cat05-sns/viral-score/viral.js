...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function evaluate() {
    ...
    if (score >= 75) {
      ...
      rageBadge.className = "inline-block px-3 py-1 bg-rose-500/20 text-rose-300 border border-rose-500/30 rounded-full text-sm font-bold font-mono animate-pulse";
      ...
      rageBadge.className = "inline-block px-3 py-1 bg-yellow-500/20 text-yellow-300 border border-yellow-500/30 rounded-full text-sm font-bold font-mono";
      rageDesc.textContent = "• 적당한 도파민 자극과 대화 참여를 유도하는 이상적인 바이럴 포스팅입니다.";
    } else {
      rageBadge.textContent = "순한맛: 평화로운 힐링 일기";
      rageBadge.className = "inline-block px-3 py-1 bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 rounded-full text-sm font-bold font-mono";
